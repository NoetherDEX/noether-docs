...
description: "The gateway authenticates requests with API keys. You get a key by proving control of a Stellar wallet: request a challenge, sign it with the wallet, and exchange the…"
---
...
Base URL for all examples: `https://noether-api.proudmeadow-533cf0d8.germanywestcentral.azurecontainerapps.io` (Stellar testnet deployment).
...
If you'd rather not script the challenge flow, the web app does it for you: connect your wallet at [https://testnet.noether.exchange/api-keys](https://testnet.noether.exchange/api-keys), approve the signing prompt (it is the same never-submitted placeholder transaction), and copy the key pair from the screen. The same closed-beta gating applies.
...
- [API explorer](/developers/reference) — try endpoints interactively

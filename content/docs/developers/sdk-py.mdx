---
title: "Python SDK"
description: "noether-sdk on PyPI is the official async Python client for the Noether gateway: REST sub-clients for markets, oracle prices, accounts, order preparation and transaction…"
---

`noether-sdk` on PyPI is the official async Python client for the Noether gateway: REST sub-clients for markets, oracle prices, accounts, order preparation and transaction submission, plus a reconnecting WebSocket client. This page documents the published **0.1.1** release against the live gateway at `https://noether-api.proudmeadow-533cf0d8.germanywestcentral.azurecontainerapps.io` (Stellar testnet, test funds).
...
- in the app at [noether.exchange/api-keys](https://testnet.noether.exchange/api-keys) (wallet sign-in, closed beta), or
...
async def main():
    async with NoetherClient("https://noether-api.proudmeadow-533cf0d8.germanywestcentral.azurecontainerapps.io") as client:
...
Prerequisites: an API key (see the callout above), the `[stellar]` extra installed, and testnet USDC in your wallet — Friendbot only provides XLM, so fund collateral from the faucet at [noether.exchange/faucet](https://testnet.noether.exchange/faucet) first.

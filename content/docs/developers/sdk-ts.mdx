...
description: "noether-sdk is the official TypeScript client for the Noether gateway."
---

`noether-sdk` is the official TypeScript client for the Noether gateway. It wraps the gateway's REST endpoints and the WebSocket stream behind typed sub-clients, and handles API-key issuance, transaction preparation, and submission. This page documents the published version **0.1.1** ([npm](https://www.npmjs.com/package/noether-sdk)).

<Callout type="info">
  The live deployment runs on the Stellar **testnet** with test funds. All snippets below point at the production gateway `https://noether-api.proudmeadow-533cf0d8.germanywestcentral.azurecontainerapps.io`.
</Callout>
...
You can also mint a key in the browser with your connected wallet on the app's [API keys page](https://testnet.noether.exchange/api-keys), then paste `keyId` and `secret` into `credentials`.
...
<Callout type="warning">
  Friendbot only funds XLM. Before `open_position` will succeed, the wallet needs test USDC collateral from the app's [faucet page](https://testnet.noether.exchange/faucet) (minimum collateral is 10 USDC, i.e. `100_000_000`).
</Callout>

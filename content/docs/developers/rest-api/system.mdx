---
title: "Keys, events & health"
description: "Reference for the gateway's system endpoints: the five API-key management endpoints, the raw contract event feed (GET /v1/events), and the liveness probe (GET…"
---

Reference for the gateway's system endpoints: the five API-key management endpoints, the raw contract event feed (`GET /v1/events`), and the liveness probe (`GET /v1/health`). All paths are relative to the production base URL `https://noether-api.proudmeadow-533cf0d8.germanywestcentral.azurecontainerapps.io`.
...
Public. Returns decoded on-chain contract events from the indexer, newest first. The same underlying event rows also stream live (framed slightly differently) on the WebSocket `events` channel — see [WebSocket](/developers/websocket).
...
```bash
curl -s "https://noether-api.proudmeadow-533cf0d8.germanywestcentral.azurecontainerapps.io/v1/events?topic=funding_applied&limit=1"
```
...
- [API explorer](/developers/reference) — try these endpoints interactively
